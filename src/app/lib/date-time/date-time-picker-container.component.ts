...
import { H2TDateTime, PickerType } from './date-time.class';
import { Observable, Subject } from 'rxjs';
import { h2tDateTimePickerAnimations } from './date-time-picker.animations';
import {
    DOWN_ARROW,
    LEFT_ARROW,
    RIGHT_ARROW,
...
@Component({
    exportAs: 'h2tDateTimeContainer',
    selector: 'h2t-date-time-container',
    templateUrl: './date-time-picker-container.component.html',
    styleUrls: ['./date-time-picker-container.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    preserveWhitespaces: false,
    animations: [
        h2tDateTimePickerAnimations.transformPicker,
        h2tDateTimePickerAnimations.fadeInPicker
    ],
    host: {
        '(@transformPicker.done)': 'handleContainerAnimationDone($event)',
        '[class.h2t-dt-container]': 'h2tDTContainerClass',
        '[class.h2t-dt-popup-container]': 'h2tDTPopupContainerClass',
        '[class.h2t-dt-popup-container-dark-mode]': 'h2tDisplayMode',
        '[class.h2t-dt-dialog-container]': 'h2tDTDialogContainerClass',
        '[class.h2t-dt-inline-container]': 'h2tDTInlineContainerClass',
        '[class.h2t-dt-container-disabled]': 'h2tDTContainerDisabledClass',
        '[attr.id]': 'h2tDTContainerId',
        '[@transformPicker]': 'h2tDTContainerAnimation',
    }
})
export class H2TDateTimeContainerComponent<T>
    implements OnInit, AfterContentInit, AfterViewInit {

    @ViewChild(H2TCalendarComponent, { static: false })
    calendar: H2TCalendarComponent<T>;
    @ViewChild(H2TTimerComponent, { static: false })
    timer: H2TTimerComponent<T>;
    hourSelected: number;
    minuteSelected: number;

    public picker: H2TDateTime<T>;
    ...
    get h2tDTContainerClass(): boolean {
        return true;
    }

    get h2tDTPopupContainerClass(): boolean {
        return this.picker.pickerMode === 'popup';
    }

    get h2tDTDialogContainerClass(): boolean {
        return this.picker.pickerMode === 'dialog';
    }

    get h2tDTInlineContainerClass(): boolean {
        return this.picker.pickerMode === 'inline';
    }

    get h2tDTContainerDisabledClass(): boolean {
        return this.picker.disabled;
    }

    get h2tDTContainerId(): string {
        return this.picker.id;
    }

    get h2tDTContainerAnimation(): any {
        return this.picker.pickerMode === 'inline' ? '' : 'enter';
    }

    get h2tDisplayMode(): any {
        return this.picker.displayMode === 'dark';
    }

    inputHourValue: any;
    inputMinuteValue: any;
    constructor( private cdRef: ChangeDetectorRef,
                  private elmRef: ElementRef,
                  private pickerIntl: H2TDateTimeIntl,
    ...
    public ngAfterContentInit(): void {
        this.initPicker();
        this.inputHourValue = this.getHour();
        this.inputMinuteValue = this.getMinute();
    }
    ...
    public onSetClicked(event: any): void {
        // this.pickerMoment = this.dateTimeAdapter.setHours(this.pickerMoment, this.hourSelected);
        // this.pickerMoment = this.dateTimeAdapter.setMinutes(this.pickerMoment, this.minuteSelected);
        // this.picker.select(this.pickerMoment);
        // this.cdRef.markForCheck();
    ...
    outputHour(event){
        this.hourSelected = parseInt(event);
    }

    outputMinute(event){
        this.minuteSelected = parseInt(event);
    }
