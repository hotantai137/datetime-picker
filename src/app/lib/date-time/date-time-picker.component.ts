...
import { H2TDialogRef } from '../dialog/dialog-ref.class';
import { H2TDialogService } from '../dialog/dialog.service';
import { merge, Subscription } from 'rxjs';
import { filter, take } from 'rxjs/operators';

/** Injection token that determines the scroll handling while the dtPicker is open. */
export const H2T_DTPICKER_SCROLL_STRATEGY = new InjectionToken<
    () => ScrollStrategy
>('h2t-dtpicker-scroll-strategy');

/** @docs-private */
export function H2T_DTPICKER_SCROLL_STRATEGY_PROVIDER_FACTORY(
    overlay: Overlay
): () => BlockScrollStrategy {
...
/** @docs-private */
export const H2T_DTPICKER_SCROLL_STRATEGY_PROVIDER = {
    provide: H2T_DTPICKER_SCROLL_STRATEGY,
    deps: [Overlay],
    useFactory: H2T_DTPICKER_SCROLL_STRATEGY_PROVIDER_FACTORY
};

@Component({
    selector: 'h2t-date-time',
    exportAs: 'h2tDateTime',
    templateUrl: './date-time-picker.component.html',
    styleUrls: ['./date-time-picker.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    preserveWhitespaces: false
})
export class H2TDateTimeComponent<T> extends H2TDateTime<T>
    ...
    private pickerContainerPortal: ComponentPortal<
        H2TDateTimeContainerComponent<T>
    >;
    private pickerContainer: H2TDateTimeContainerComponent<T>;
    private popupRef: OverlayRef;
    private dialogRef: H2TDialogRef<H2TDateTimeContainerComponent<T>>;
    ...
    private _dtInput: H2TDateTimeInputDirective<T>;
    get dtInput() {
    ...
    constructor(
        ...
        private dialogService: H2TDialogService,
        private ngZone: NgZone,
        protected changeDetector: ChangeDetectorRef,
        @Optional() protected dateTimeAdapter: DateTimeAdapter<T>,
        @Inject(H2T_DTPICKER_SCROLL_STRATEGY) defaultScrollStrategy: any,
        @Optional()
    ...
    public registerInput(input: H2TDateTimeInputDirective<T>): void {
        if (this._dtInput) {
            throw Error(
                'A H2T DateTimePicker can only be associated with a single input.'
            );
        }
    ...
    private openAsDialog(): void {
        this.dialogRef = this.dialogService.open(
            H2TDateTimeContainerComponent,
            {
                ...
                paneClass: ['h2t-dt-dialog', ...coerceArray(this.panelClass)],
    ...
    private openAsPopup(): void {
        if (!this.pickerContainerPortal) {
            this.pickerContainerPortal = new ComponentPortal<
                H2TDateTimeContainerComponent<T>
            >(H2TDateTimeContainerComponent, this.viewContainerRef);
        }
    ...
    private createPopup(): void {
        const overlayConfig = new OverlayConfig({
            ...
            panelClass: ['h2t-dt-popup', ...coerceArray(this.panelClass)]
        });
    ...
    private createPopupPositionStrategy(): PositionStrategy {
        return this.overlay
            ...
            .withTransformOriginOn('.h2t-dt-container')
