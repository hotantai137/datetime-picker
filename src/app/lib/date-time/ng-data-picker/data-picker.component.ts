...
export class DataPickerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('pickerHandleLayer', {static: true}) pickerHandleLayer
  ...
  triggerBelH2TayerClick (ev, gIndex) {
    const movedIndex = this.currentIndexList[gIndex] - 1
    this.currentIndexList[gIndex] = movedIndex
    this.correctionCurrentIndex(ev, gIndex)
  }
  ...
  handleEventClick (ev) {
    ...
    switch (ev.target.dataset.type) {
      ...
      case 'middle':
        ...
        break
      case 'bottom':
        this.triggerBelH2TayerClick(ev, gIndex)
        break
      default:
    }
  }
  ...
  setCurrentIndexOnMove (ev) {
    ...
    this.currentIndexList[gIndex] = movedIndex
    this.draggingInfo.startPageY = touchInfo.pageY
    this.swapArray(0, this.currentIndexList[0]);
  }
  ...
  swapArray(groupIndex, itemIndex){
    let dataGroup = this.data[groupIndex].list;
    if(itemIndex < 23){
      let newArray = dataGroup.slice(48, 72);
      dataGroup = dataGroup.slice(0, 48);
      this.data[groupIndex].list = newArray.concat(dataGroup);
    }
  }
}
