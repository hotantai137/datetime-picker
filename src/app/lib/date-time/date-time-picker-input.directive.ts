...
export const H2T_DATETIME_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => H2TDateTimeInputDirective),
    multi: true
};

export const H2T_DATETIME_VALIDATORS: any = {
...
@Directive({
    selector: 'input[h2tDateTime]',
    exportAs: 'h2tDateTimeInput',
    host: {
        '(keydown)': 'handleKeydownOnHost($event)',
        '(blur)': 'handleBlurOnHost($event)',
        '(input)': 'handleInputOnHost($event)',
        '(change)': 'handleChangeOnHost($event)',
        '[attr.aria-haspopup]': 'h2tDateTimeInputAriaHaspopup',
        '[attr.aria-owns]': 'h2tDateTimeInputAriaOwns',
        '[attr.min]': 'minIso8601',
        '[attr.max]': 'maxIso8601',
        '[disabled]': 'h2tDateTimeInputDisabled'
    },
    providers: [
        H2T_DATETIME_VALUE_ACCESSOR,
        H2T_DATETIME_VALIDATORS,
    ],
})
export class H2TDateTimeInputDirective<T>
    implements
        OnInit,
        AfterContentInit,
        OnDestroy,
     ...
     * */
    @Input()
    set h2tDateTime(value: H2TDateTimeComponent<T>) {
     ...
     */
    @Input()
    set h2tDateTimeFilter(filter: (date: T | null) => boolean) {
    ...
    /** The date-time-picker that this input is associated with. */
    public dtPicker: H2TDateTimeComponent<T>;
    ...
    private parseValidator: ValidatorFn = (): ValidationErrors | null => {
        return this.lastValueValid
            ? null
            : { h2tDateTimeParse: { text: this.elmRef.nativeElement.value } };
    };
    ...
    private minValidator: ValidatorFn = (
        ...
        if (this.isInSingleMode) {
            ...
            return !this.min ||
                ...
                ? null
                : { h2tDateTimeMin: { min: this.min, actual: controlValue } };
    ...
    private maxValidator: ValidatorFn = (
        ...
        if (this.isInSingleMode) {
            ...
            return !this.max ||
                ...
                ? null
                : { h2tDateTimeMax: { max: this.max, actual: controlValue } };
        } else if (this.isInRangeMode && control.value) {
            const controlValueFrom = this.getValidDate(
                this.dateTimeAdapter.deserialize(control.value[0])
            );
    ...
    get h2tDateTimeInputAriaHaspopup(): boolean {
        return true;
    }

    get h2tDateTimeInputAriaOwns(): string {
    ...
    get h2tDateTimeInputDisabled(): boolean {
    ...
    constructor( private elmRef: ElementRef,
        ...
        @Optional() @Inject(H2T_DATE_TIME_FORMATS) private dateTimeFormats: H2TDateTimeFormats ) {
        if (!this.dateTimeAdapter) {
            throw Error(
                `H2TDateTimePicker: No provider found for DateTimePicker. You must import one of the following ` +
                    `modules at your application root: H2TNativeDateTimeModule, H2TMomentDateTimeModule, or provide a ` +
        ...
        if (!this.dateTimeFormats) {
            throw Error(
                `H2TDateTimePicker: No provider found for H2T_DATE_TIME_FORMATS. You must import one of the following ` +
                    `modules at your application root: H2TNativeDateTimeModule, H2TMomentDateTimeModule, or provide a ` +
                    `custom implementation.`
            );
        }
    ...
    public ngOnInit(): void {
        if (!this.dtPicker) {
            throw Error(
                `H2TDateTimePicker: the picker input doesn't have any associated h2t-date-time component`
            );
        }
    }
     ...
     */
    private registerDateTimePicker(picker: H2TDateTimeComponent<T>) {
        if (picker) {
